import { assertSiteKey, corsHeaders, jsonResponse, proxyJsonRequest } from "./_shared/proxy";
...
const DEFAULT_MODEL = (process.env.AFRICHAT_MODEL || "gpt-5.4").trim() || "gpt-5.4";
const DEFAULT_REASONING = (process.env.AFRICHAT_REASONING_EFFORT || "medium").trim() as "none" | "low" | "medium" | "high";

function buildSystemPrompt(): string {
  const custom = (process.env.AFRICHAT_SYSTEM_PROMPT || "").trim();
